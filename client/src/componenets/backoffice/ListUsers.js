...
function ListUsers() {
  ...
  useEffect(() => {
    ...
    // Fetch user role
    axios.get("http://localhost:5000/api/users/getUserRole", {
      headers: {
...
useEffect(() => {
 ...
 const handleSearch = () => {
  const filteredResults = users.filter((user) => {
    ...
    const firstNameMatch = user.firstName && user.firstName.toLowerCase().includes(searchTerm.toLowerCase());
    const lastNameMatch = user.lastName && user.lastName.toLowerCase().includes(searchTerm.toLowerCase());
    const roleMatch = user.role && user.role.toLowerCase().includes(searchTerm.toLowerCase());
    const subscriptionTypeMatch = user.subscriptionType && user.subscriptionType.toLowerCase().includes(searchTerm.toLowerCase());

    return usernameMatch || emailMatch || firstNameMatch || lastNameMatch || roleMatch || subscriptionTypeMatch;
  });
  ...
  const handleEditFormSubmit = (e) => {
    ...
    if (validateEditForm()) {
      ...
      const emailExists = users.some(user => user.email === formData.email && user._id !== editingUser._id);
      const usernameExists = users.some(user => user.username === formData.username && user._id !== editingUser._id);

  
      if (emailExists) {
        setEditErrorMessages({ email: 'This email already exists. Please choose a different one.' });

      } if (usernameExists) {
        setEditErrorMessages({ username: 'This username already exists. Please choose a different one.' });
        }
        else {
...
//PAGINATION CODE START
  ...
  const currentUsers = users.slice(indexOfFirstUser, indexOfLastUser);
   ...
   const pageLinks = Array.from(
     ...
     (_, index) => (
       <li key={index + 1} className={currentPage === index + 1 ? 'page-item active' : 'page-item'}>
  ...
  const emptyRows = Array.from({ length: emptyRowsCount }, (_, index) => (
    <tr key={`empty-row-${index}`}>
      <td colSpan="8">{/* Adjust the colspan based on the number of columns in your table */}</td>
    </tr>
  ));
    ...
    <div className="modal-dialog">
      ...
      <form onSubmit={handleEditFormSubmit}>
          <div className="modal-body">
            ...
            {editErrorMessages.username && <p className='error'>{editErrorMessages.username}</p>}</div>
          </div>
          <div className="form-group">
            <label>Email</label>
            <input
              ...
              required
            />
            {editErrorMessages.email && <p className='error'>{editErrorMessages.email}</p>}
          </div>
            <div className="form-group">
