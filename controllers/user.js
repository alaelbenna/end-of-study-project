...
exports.Login = async (req, res) => {
  ...
  exports.updateUserById = async (req, res) => {
    ...
    try {
      ...
      if ('email' in updates) {
        const existingEmailUser = await User.findOne({ email: updates.email, _id: { $ne: userId } });
      ...
      const allowedUpdates = ['username', 'email', 'firstName', 'lastName', 'subscriptionType'];
      const updateFields = {};
      ...
      if (Object.keys(updateFields).length > 0) {
        user = await User.findByIdAndUpdate(userId, { $set: updateFields }, { new: true });
        if (!user) {
          return res.status(404).json({ error: 'User not found' });
        }
      }
